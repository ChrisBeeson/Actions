...
public class TransitionNodeView: NSView {
    ...
    override public func drawRect(dirtyRect: NSRect) {
        ...
        path.moveToPoint(NSPoint(x: 0 , y: (frame.size.height/2)-(path.lineWidth/2)+1))
        path.lineToPoint(NSPoint(x: frame.size.width , y:(frame.size.height/2)-(path.lineWidth/2)+1))
        path.stroke()
    }
}
